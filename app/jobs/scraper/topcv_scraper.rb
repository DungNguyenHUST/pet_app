module TopcvScraper
    ...
    def get_summary_data_topcv(scrap_job)
        if check_exist_url(scrap_job.url.to_s)
            ...
            processing_summary_blocks = doc.css("div.job-ta")
            processing_summary_datas = []

            if processing_summary_blocks.present?
                processing_summary_blocks.each do |processing_summary_block|
                    unless processing_summary_block.css("h4.job-title a").nil?
                        job_link = processing_summary_block.css("h4.job-title a").map { |link| link['href']}.first
                    end

                    unless processing_summary_block.css("span.address").nil?
                        job_location = processing_summary_block.css("span.address").text.strip
                    end
    ...
    def get_detail_data_topcv(scrap_job)
        job_summary_params = get_summary_data_topcv(scrap_job)
